...
[  -z $Silent ] && echo -n "Testing connection to the DMTF site..."
...
if [ $# -ne  $NUMARGS -a $# -ne $NUMMINARGS ] ; then
   echo -e "Usage: $0 [bmc-IP bmc-user password] collection\n"
...
# Create BMC session and collecte the Token and the Session location 
...
while [[ "$CollectionURIs" =~ "/{" ]] ; do
	...
	for uri in $CollectionURIs ; do
	    if [[ ! "$uri" =~ "/{" ]] ; then
		    ...
		    # $uri contains {}. Adding it to TempList.
		    TempList="${uri}"
		fi
	done
